...
def _(
    ...
    e: TypeGuard,  # error: [invalid-type-form] "`TypeGuard` requires exactly one argument when used in a type expression"
    f: TypeIs,  # error: [invalid-type-form] "`TypeIs` requires exactly one argument when used in a type expression"
):
