...
impl<'db> Node<'db> {
    ...
    fn for_each_path(
        ...
        let mut present_ordered = Vec::new();
        self.for_each_constraint(db, &mut |constraint| {
            if present.insert(constraint) {
                present_ordered.push(constraint);
            }
        });
        present_ordered.sort_unstable_by(|left, right| left.ordering(db).cmp(&right.ordering(db)));
        let mut support_order = support.clone();
        for constraint in present_ordered {
            if !support_order.contains(&constraint) {
                support_order.insert(constraint);
            }
        }
        let missing = support
            .iter()
        ...
        let mut path = match self {
            Node::Interior(interior) => interior.path_assignments(db, &support_order),
            Node::AlwaysTrue | Node::AlwaysFalse => {
                PathAssignments::new_with_support(db, &support_order)
            }
        };
    ...
    fn for_each_path_with_missing(
        ...
        if let Some((constraint, rest)) = missing.split_first() {
            ...
            let _ = path.walk_edge(db, constraint.when_true(), source_order, |path, _| {
                self.for_each_path_with_missing(db, rest, f, path);
            });
            let _ = path.walk_edge(db, constraint.when_false(), source_order, |path, _| {
                self.for_each_path_with_missing(db, rest, f, path);
            });
        } else {
...
impl<'db> InteriorNode<'db> {
    ...
    fn sequent_map_with_support(
        ...
        constraints.sort_unstable_by(|left, right| {
            let left_index = support.get_index_of(left);
            let right_index = support.get_index_of(right);
            match (left_index, right_index) {
                (Some(left_index), Some(right_index)) => left_index.cmp(&right_index),
                (Some(_), None) => Ordering::Less,
                (None, Some(_)) => Ordering::Greater,
                (None, None) => left.ordering(db).cmp(&right.ordering(db)),
            }
        });
